...
public class BSPBuilder {
	 ...
	 * @param colchange column change
	 * @param expectedPartiters expected part iters
	 */
	...
	private BSPNode genNodes(ArrayList<Seg> sl) {
		...
		final int x  = pe.getX();
		final int y  = pe.getY();
		final int dx = pe.getDx();
		final int dy = pe.getDy();
		...
		for (int i = 0; i != sl.size(); i++) {
			...
			int df1x = se.getX() - x;
			int df1y = se.getY() - y;
			int sendx = se.getX() + se.getDx();
			int sendy = se.getY() + se.getDy();
			int df2x = sendx - x;
			int df2y = sendy - y;
			int nx = dy;
			int ny = -dx;
			...
			if (getSign(dot1) != getSign(dot2)) {
				if (dot1 == 0)
					...
					if (dx == 0)
						...
						spy = y;
					Seg sps1 = new Seg(se.getX(), se.getY(), spx-se.getX(), spy-se.getY(), se.getDist(), colchange);
					Seg sps2 = new Seg(spx, spy, sendx-spx, sendy-spy, se.getDist(), colchange);
					if (dot1 > 0) {
						rsl.add(sps1);
						lsl.add(sps2);
					} else {
					...
					sps1.setPartition(se.getPartition());
					sps2.setPartition(se.getPartition());
					continue;
				}
			}
	...
	private void setPartitionBitForCertainSegments(ArrayList<Seg> sl) {
		for (int i = 0; i != sl.size(); i++) {
			Seg se = sl.get(i);
			if (((se.getX() == 0 || se.getX() == width ) && se.getDx() == 0) ||
					((se.getY() == 0 || se.getY() == height) && se.getDy() == 0))
				se.setPartition(true);
		}
	}
	...
	private int grade_partition(ArrayList<Seg> sl, Seg pe) {
		...
		for (int i = 0; i < sl.size(); i += inc) {
			Seg se = (Seg) sl.get(i);
			int df1x = se.getX()-x; // difference between beginning of segment and x
			int df1y = se.getY()-y; // difference between beginning of segment and y
			int sendx = se.getX() + se.getDx(); // end of segment
			int sendy = se.getY() + se.getDy(); // end of segment
