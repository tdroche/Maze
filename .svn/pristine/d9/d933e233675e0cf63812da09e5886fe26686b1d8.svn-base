...
public class MazeFileReader {
	 ...
	 */
	public MazeFileReader(InputStream inputFile) {
		load(inputFile);
	}
	 ...
	 * @param inputStream the maze file input stream
	 */
	...
	private void load(InputStream inputStream) {
		try {
			...
			NodeList nodeList = document.getElementsByTagName("Maze");
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (node.getNodeType() == Node.ELEMENT_NODE) {
					...
					width = getElementIntValue("sizeX", element);
					height = getElementIntValue("sizeY", element);
					rooms =  getElementIntValue("roomNum", element);
					dists = new int[width][height];
					expected_partiters = getElementIntValue("partiters", element);
					cells = readCells(element);

					// Read array of distance values.

					readDistances(element);

					// Read start position.

					startx = getElementIntValue("startX", element);
					starty = getElementIntValue("startY", element);

					// Read tree of BSPNodes.

					number = 0; // field used as an index, that is shared an updated across recursive readBSPNode calls
					root = readBSPNode(element);
				}
			}
		}
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		result.setCol(GraphicsWrapper.returnColorArray(col));
