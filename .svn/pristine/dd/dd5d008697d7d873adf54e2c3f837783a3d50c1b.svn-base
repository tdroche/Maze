...
public class MapDrawer extends DefaultViewer implements Serializable {
	private static final long serialVersionUID = 4618615074619972778L;
	...
	private int mazew ; // width 
	private int mazeh ; // height
	...
	public void redraw(GraphicsWrapper wrapper, int state, int px, int py,
		...
		//	return ;
		if (maze.isInMapMode()) {
			draw_map(wrapper, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
			draw_currentlocation(wrapper, view_dx, view_dy) ;
		}
	}
	/**
	 ...
	 */
	public void draw_map(GraphicsWrapper wrapper, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		wrapper.setWhite();
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				if ((seencells.hasWallOnLeft(x, y) || showMaze) && w)
					wrapper.drawLine(nx1, ny1, nx1, ny2);
			}
		if (showSolution) {
	 ...
	 * @param gc
	 */
	public void draw_currentlocation(GraphicsWrapper wrapper, int view_dx, int view_dy) {
		wrapper.setRed();
	 ...
	 * @param offy
	 */
	public void draw_solution(GraphicsWrapper wrapper, int offx, int offy, int px, int py) {
		// check parameters:
		...
		while (d > 1) {
			...
			// do the graphics
			wrapper.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
