...
public class FirstPersonDrawer extends DefaultViewer implements Serializable {
	private static final long serialVersionUID = 9191617506428657250L;
	...
	private Cells mazecells ; // current maze
	...
	private final int viewz = 50;  // constant from Maze.java
	 ...
	 */
	@Override
	public void redraw(GraphicsWrapper wrapper, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		wrapper.setDarkGray();
		wrapper.fillRect(0, view_height/2, view_width, view_height/2);
		...
		setTraverse_node_ct(0);
		setTraverse_ssector_ct(0);
		setDrawrect_ct(0);
		setDrawrect_late_ct(0);
		setDrawrect_segment_ct(0);
	...
	private void drawAllSegmentsOfASector(BSPLeaf n) {
		...
		for (int i = 0; i != sl.size(); i++) {
			...
			int v1x = seg.getX();
			int v1y = seg.getY();
			int v2x = v1x+seg.getDx();
			int v2y = v1y+seg.getDy();
			// draw rectangle
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		wrapper.setGraphicsColor(seg);
		boolean drawn = false;

		drawrect_late_ct++; // debug, counter
		// setDrawrect_late_ct(getDrawrect_late_ct() + 1); // debug, counter
	...
	private void udpateSeenCellsForSegment(Seg seg) {
		...
		final int sdx = seg.getDx() / map_unit; // constant, only set once here
		final int sdy = seg.getDy() / map_unit; // constant, only set once here

		// Step 2: get initial position (sx,sy) right
		int sx = seg.getX() / map_unit;
		if (sdx < 0)
			sx--;
		int sy = seg.getY() / map_unit; 
		if (sdy < 0)
			sy--;
	...
	public void setMazedists(int[][] mazedists) {
		this.mazedists = mazedists;
	}
	...
	public int getTraverse_node_ct() {
		return traverse_node_ct;
	}

	public void setTraverse_node_ct(int traverse_node_ct) {
		this.traverse_node_ct = traverse_node_ct;
	}

	public int getTraverse_ssector_ct() {
		return traverse_ssector_ct;
	}

	public void setTraverse_ssector_ct(int traverse_ssector_ct) {
		this.traverse_ssector_ct = traverse_ssector_ct;
	}

	public int getDrawrect_ct() {
		return drawrect_ct;
	}

	public void setDrawrect_ct(int drawrect_ct) {
		this.drawrect_ct = drawrect_ct;
	}

	public int getDrawrect_late_ct() {
		return drawrect_late_ct;
	}

	public void setDrawrect_late_ct(int drawrect_late_ct) {
		this.drawrect_late_ct = drawrect_late_ct;
	}

	public int getDrawrect_segment_ct() {
		return drawrect_segment_ct;
	}

	public void setDrawrect_segment_ct(int drawrect_segment_ct) {
		this.drawrect_segment_ct = drawrect_segment_ct;
	}
}
