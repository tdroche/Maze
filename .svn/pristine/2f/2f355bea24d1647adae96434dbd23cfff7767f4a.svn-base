...
public class RangeSet implements Serializable {
	private static final long serialVersionUID = 719934975674986408L;
	...
	/**
	 * Tells if there is at least one interval that intersects with [point.x,point.y]. It modifies attributes of point in case of an intersection. 
	 * A point is used as a quick hack to communicate two integers and being able to return modified values in a boolean method.
	 * @param point describes interval [point.x,point.y], its entries are modified to represent the intersection if there is one
	 * @return true if there is an intersection, false otherwise
	 */
	public boolean intersect(int[] point) {
		// "point" isn't (strictly speaking) a point, but I need to return two
